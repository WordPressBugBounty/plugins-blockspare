...
class Blockspare_NewSpare_Notice extends Blockspare_Setup {
    ...
    public function __construct() {
        ...
        // Process dismiss actions before checking conditions
        $this->process_dismiss_actions();
    ...
    /**
     * Listen and process the dismiss actions when a user clicks them.
     */
    private function process_dismiss_actions() {
        ...
        if ( isset( $_GET['blockspare_setup_notice_dismiss'] ) && 'newspare' === $_GET['blockspare_setup_notice_dismiss'] ) {
            if ( isset( $_GET['_blockspare_setup_notice_dismiss_nonce'] ) && wp_verify_nonce( $_GET['_blockspare_setup_notice_dismiss_nonce'], 'blockspare_setup_notice_dismiss_nonce' ) ) {
                update_user_meta( $user_id, 'blockspare_newspare_notice_dismiss', 'yes' );
                wp_safe_redirect( remove_query_arg( array( 'blockspare_setup_notice_dismiss', '_blockspare_setup_notice_dismiss_nonce' ) ) );
                exit;
            }
        }

        // Handle Temporary Dismiss
        if ( isset( $_GET['blockspare_setup_notice_dismiss_temporary'] ) && 'newspare' === $_GET['blockspare_setup_notice_dismiss_temporary'] ) {
            if ( isset( $_GET['_blockspare_setup_notice_dismiss_temporary_nonce'] ) && wp_verify_nonce( $_GET['_blockspare_setup_notice_dismiss_temporary_nonce'], 'blockspare_setup_notice_dismiss_temporary_nonce' ) ) {
                update_user_meta( $user_id, 'blockspare_newspare_notice_dismiss_temporary_start_time', time() );
    ...
    public function set_dismiss_notice() {
        ...
        $is_permanently_dismissed = ( 'yes' === get_user_meta( $user_id, 'blockspare_newspare_notice_dismiss', true ) );
        
        $temporary_dismiss_time   = (int) get_user_meta( $user_id, 'blockspare_newspare_notice_dismiss_temporary_start_time', true );
        $is_temporarily_dismissed = ( $temporary_dismiss_time > strtotime( '-7 days' ) );

        // If permanently or temporarily dismissed, hide the notice
        if ( $is_permanently_dismissed || $is_temporarily_dismissed ) {
            add_filter( 'blockspare_newspare_notice_dismiss', '__return_true' );
        } else {
            add_filter( 'blockspare_newspare_notice_dismiss', '__return_false' );
        }
    }
