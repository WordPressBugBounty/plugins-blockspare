  ...
  function blockspare_dashboard_widget_content()
  {
    ...
    <div class="bs-welcome-panel-content">
      <div class='bs-welcome-panel-banner'>
        <h2><?php esc_html_e('Effortless Site Creation in Minutes: Expert Templates and Blocks for Your Blog, News, Magazine, and Agency Websites!', 'blockspare'); ?></h2>
      </div>
      <div class='bs-welcome-panel-header'>

        <div class='bs-welcome-panel-header-logo'>
          <svg fill="url(#blockspare-gradient)" width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40" class="blockspare-svg blockspare-svg-blockspare" aria-hidden="true" focusable="false">
            <g class="blockspare-logo">
              <g>
                <path class="st0" d="M1.6,9.2v21.4l18.3-9.2L20.2,0L1.6,9.2z M16.9,19.8L4.9,26V10.9l12-6.1V19.8z"></path>
                <polygon id="XMLID_3_" class="st1" points="19.9,21.4 16.9,23 26,27.7 13.8,33.8 4.9,29 1.6,30.7 13.9,36.9 32.4,27.7"></polygon>
              </g>
              <g>
                <polygon id="XMLID_2_" class="st0" points="23,1.5 23,19.8 32.4,24.6 32.4,9.4 35.3,10.9 35.3,29 38.4,30.7 38.4,9.2 29.4,4.8  29.2,19.7 26,18.4 26,3.1"></polygon>
                <polygon id="XMLID_1_" class="st1" points="17,38.4 19.9,40 38.4,30.7 35.3,29"></polygon>
              </g>
            </g>
          </svg>
        </div>
      ...
      <div class='bs-welcome-panel-column-container'>
        ...
        <div class='bs-welcome-panel-column'>
          ...
          <div class='bs-welcome-panel-column-content'>
            ...
            <a href="<?php echo esc_url('https://www.blockspare.com/') ?>" target="_blank"><?php esc_html_e('Enhance Your Plan', 'blockspare'); ?></a>
          </div>
        </div>
...
function blockspare_get_latest_change_log()
{
  $cache_key = 'blockspare_changelog_all_' . BLOCKSPARE_VERSION;
  ...
  $readme = BLOCKSPARE_PLUGIN_DIR . 'readme.txt';
  ...
  // Extract the entire == Changelog == section to ensure we aren't pulling from other sections
  if (!preg_match('/== Changelog ==\s*(.*)/s', $content, $changelog_section)) {
    return [];
  }

  $changelog_text = $changelog_section[1];

  // Regex to find ALL version blocks
  // Matches "= 7.2.0 =" into group 1, and everything until the next version header into group 2
  $pattern = '/=\s*([0-9\.]+)\s*=\s*(.*?)(?==\s*[0-9\.]+\s*=|\z)/s';

  if (!preg_match_all($pattern, $changelog_text, $matches, PREG_SET_ORDER)) {
    return [];
  }
  ...
  foreach ($matches as $match) {
    ...
    $raw_items = trim($match[2]);
    $changes = [];

    // Extract bullet points for this specific version block
    preg_match_all('/^\* (.+)$/m', $raw_items, $items);

    if (!empty($items[1])) {
      $changes = array_map('trim', $items[1]);
    }

    // Only add if there are items or if you want to include empty versions
    $all_data[] = [
      'version' => $version,
      'items'   => $changes,
    ];
  ...
  // Cache forever (until version changes).
  set_transient($cache_key, $all_data, YEAR_IN_SECONDS);
